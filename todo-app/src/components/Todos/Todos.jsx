import React from 'react'
import './Todos.css'
import { useSelector } from 'react-redux'
import store from '../../store/store'
import { useDispatch } from 'react-redux'
import { deleteTodo } from '../../actions/todos'

function Todos() {

  const todos=useSelector(store=>store.todos)

  /* const todos=[
    "Revise Advanced Javascript",
    "Go for a 30 mins walk",
    "Revise Pseudo class selectors in CSS"
  ] */
  const dispatch=useDispatch()
    //capture the index value from handleDeleteTodo(index)->todoIndex
    const handleDeleteTodo=(todoIndex)=>{
      dispatch(deleteTodo(todoIndex))
    }

  return (
    <div id='todo-container'>
      {todos.map((todo,index) => (
        <div className='todo-item' key={index}>{/*key prop for parent element to avoid error (for map function) */}
          
          <span>{todo}</span>
          <span
            id='clossBtn'
            onClick={()=>handleDeleteTodo(index)}
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-file-x" viewBox="0 0 16 16">
  <path d="M6.146 6.146a.5.5 0 0 1 .708 0L8 7.293l1.146-1.147a.5.5 0 1 1 .708.708L8.707 8l1.147 1.146a.5.5 0 0 1-.708.708L8 8.707 6.854 9.854a.5.5 0 0 1-.708-.708L7.293 8 6.146 6.854a.5.5 0 0 1 0-.708"/>
  <path d="M4 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2zm0 1h8a1 1 0 0 1 1 1v12a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1"/>
</svg>
  </span>
        </div>
      ))}
    </div>
  )
}

export default Todos
